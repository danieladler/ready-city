class Preparation < ApplicationRecord
  ...
  # end
    ...
    def self.import(file)
      ...
      if @preparation_keywords_to_clear.count != 0
        clear_old_db_preparations(@preparation_keywords_to_clear)   # remove from DB any record w/ keyword that does not exist in CSV
      end
    ...
    #     end
    #
    #     # if Preparation.find_by(keyword: row["keyword"]) == nil
    #     #   Preparation.create row.to_hash
    #     # else
    #     #   @preparation = Preparation.find_by(keyword: row["keyword"])
    #     #   @preparation.update(row.to_hash)
